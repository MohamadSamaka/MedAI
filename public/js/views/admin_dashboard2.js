        ...
        <nav class="navbar">
          <ul id="navbarList">
            <li id="mang_btn">
                ...
                <p style="color:white; margin-left:10px;"><i class="fa-solid fa-users"></i> User Management</p
              >
            </li>
            <li id="stats_btn">
              
                   <p style="color:white; margin-left:10px;"><i class="fa-solid fa-chart-line"></i> Stats</p>
...
export async function init(styles, params) {
  ...
  try {
    expertise_array = await getExpertise();
    console.log("expertise_array == ", expertise_array);
  } catch (error) {
    alert(error.message);
  }

  try {
    locations_array = await getLocations();
    console.log("locations_array == ", locations_array);
  } catch (error) {
    alert(error.message);
  }
  ...
  function renderPatientDoctorStats() {
    const patientCount = roles.filter(
      (item) => item.roleName === "patient"
    ).length;

    // Count doctors
    const doctorCount = roles.filter(
      (item) => item.roleName === "doctor"
    ).length;

    console.log("===patientCount=", doctorCount);
  ...
  function renderDoctorStats(category) {
    ...
    // const expertiseNames = expertise_array.map(expertise => expertise.name);
    ...
    if (category === "expertise") {
      expertise_array.forEach((expertise) => {
        const name = expertise.name;

        // If the name already exists as a key, increment its count
        if (counts[name]) {
          counts[name]++;
        } else {
  ...
  var liElement = document.getElementById("stats_btn");

  // Set the onclick event dynamically
  liElement.onclick = function () {
    showStats();
  };

  var liElement = document.getElementById("mang_btn");
